...
*/
    ...
    } elseif (file_exists('/usr/local/prospamfilter')) {
...
class Uninstaller
{
	...
	private function panel_actions()
	{
		...
		switch( $paneltype )
		{
			case "CPANEL":
				...
				// Remove postwwwacct  hook (WHM)
				$this->remove_hook('/scripts/postwwwacct', 'postwwwacct','/usr/local/prospamfilter/bin/hook.php action adddomain "$@"');

				// Remove prekillacct hook (WHM)
				$this->remove_hook('/scripts/prekillacct', 'prekillacct','/usr/local/prospamfilter/bin/hook.php action predelaccount "$@"');

				// Remove Park hook (CPANEL)
				$this->remove_hook('/usr/local/cpanel/hooks/park/park', 'park','/usr/local/prospamfilter/bin/hook.php "$@"');

				// Remove Unpark hook (CPANEL)
				$this->remove_hook('/usr/local/cpanel/hooks/park/unpark', 'unpark','/usr/local/prospamfilter/bin/hook.php "$@"');

				// Remove AddAddonDomain hook (CPANEL)
				$this->remove_hook('/usr/local/cpanel/hooks/addondomain/addaddondomain', 'addondomain','/usr/local/prospamfilter/bin/hook.php "$@"');

				// Remove DelAddonDomain hook (CPANEL)
				$this->remove_hook('/usr/local/cpanel/hooks/addondomain/deladdondomain', 'deladdondomain','/usr/local/prospamfilter/bin/hook.php "$@"');

				// Remove savecontactinfo hook (CPANEL)
				$this->remove_hook('/usr/local/cpanel/hooks/custinfo/savecontactinfo', 'addondomain','/usr/local/prospamfilter/bin/hook.php "$@"');

				// Remove setmxcheck hook (CPANEL)
				$this->remove_hook('/usr/local/cpanel/hooks/email/setmxcheck', 'setmxcheck','/usr/local/prospamfilter/bin/hook.php "$@"');

				// Unregister hooks with cPanel
	...
	{
		$version = trim(file_get_contents(dirname(__DIR__) . DS . "application" . DS . "version.txt"));
